...
export const Weather = () => {
  ...
  const { data, status } = useFetch<any>(
    import.meta.env.VITE_NAJWER23API_FASTIFY_ORIGIN + `/openweathermap/forecast/onecall?lat=${lat}&lon=${lon}`,
    {
      method: 'GET',
      credentials: 'include',
      headers: {
        ...
        {status === 'done' ? (
          <Carousel
            ...
            <div className={styles.weatherCurrent}>
              ...
              <WeatherForecastData
                title={'Sunrise'}
                value={dateFormatterFromDt(weatherCurrent!.sunrise)?.split(',')[1] ?? ''}
              />
              <WeatherForecastData
                title={'Sunset'}
                value={dateFormatterFromDt(weatherCurrent!.sunset)?.split(',')[1] ?? ''}
              />
              ...
              weather8Days
                ...
                .map((x: any) => (
                  <div key={x.dt}>
                    ...
                    <WeatherForecastData
                      title={'Sunrise'}
                      value={dateFormatterFromDt(x.sunrise)?.split(',')[1] ?? ''}
                    />
